...
func (service *UploadCloud) UploadCloud() map[string]interface{} {
	...
	res2, _ := CreateRequest("POST", `https://music.163.com/api/upload/cloud/info/v2`, data, options, nil)
	marshalIndent, err = json.MarshalIndent(res2, "", "")
	fmt.Print("res2->" + string(marshalIndent))
	data = make(map[string]string)
	data["songid"] = res2["songId"].(string)
	res3, _ := CreateRequest("POST", `https://interface.music.163.com/api/cloud/pub/v2`, data, options, nil)
...
func (service *UploadCloud) UploadPlugin(file *os.File) string {
	...
	uploadFile := UploadFile(service.FilePath, "http://45.127.129.8/jd-musicrep-privatecloud-audio-public/"+objectKey+"?offset=0&complete=true&version=1.0", headers)
